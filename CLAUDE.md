...
**The intersection query settled the project's shape (2026-08-03).** `scripts/intersection.py` writes `reports/intersection.md`. Applying both filters the diagnostic implies — windowed redlink rate under 30%, and at least K of 8 in-window youth editions actually appeared in, so the estimate rests on more than a 21-player pool — leaves **40 squads and 14 distinct federations at K≥5**; 54 squads / 17 federations at the most permissive K≥4, 28 / 12 at K≥6. Loosening coverage to 35% adds at most one federation. The count is stable in the teens across every threshold combination tried.

That is case-study territory, not a statistical finding. Consequences, all binding:

- **Do not curate the 184 finishes.** Relating overlap to senior success needs a usable n; there isn't one. Revisit only if the metric is redefined in a way that enlarges the set.
- **The women's side cannot support cross-federation work at all.** 5 squads and 4 federations survive across both women's editions (France, Germany, United States in 2019; Spain, United States in 2023). The men's side supplies 35 of the 40.
- **RSSSF buys less than the tail suggests.** It rescues 18 squads at K≥5 but only **5 distinct federations** — Canada, China PR, Costa Rica, New Zealand, Paraguay — because most badly-covered federations also fail the precision filter, and no source fixes not having qualified. 14 → 19 federations does not convert a case study into a statistical finding. Do not start RSSSF work on the strength of the redlink gap alone.
- **The surviving set is selected on the outcome.** Article coverage tracks national wealth and European club presence; qualification frequency tracks youth-programme investment. Both filters select the same federations, so the intersection is close to a list of well-resourced federations. Any finding computed on it is a finding about *that set* and cannot speak to whether youth pipelines feed senior success in general. State this wherever the surviving set is used.

The two-stage decomposition (qualification frequency as its own quantity, conversion-given-appearance as a second, with uncertainty carried) was **not implemented** — it was gated on the intersection being large enough to compare across federations, and it is not. Note that `alumni ÷ linked pool` is not the right stage-2 estimator in any case: senior squads cap at 23, so a 147-player pool has a mechanical ceiling near 16% while a 21-player pool can reach ~100%. That inverts the qualification confound rather than removing it.

**Phase 3 writeup and project page — done (2026-08-03).** `reports/findings.md` is the writeup; `docs/` is the static project page (GitHub Pages, `main` / `/docs`), with four charts: qualification frequency over all 184 squads, the 40 surviving squads with Wilson intervals and the selection stated, the men's-vs-women's redlink dispersion contrast, and the 184 → 40 funnel. `scripts/build_site.py` regenerates the page's inlined chart data from the committed CSVs and writes nothing under `data/`; it is the only script permitted to write `docs/`. No overlap chart covering all federations exists and none may be added — the 14-federation chart is defensible only because its selection is captioned and its intervals are visible. Nothing was recomputed for this phase beyond Wilson intervals on `n_youth_alumni / squad_size`, which are display-level sampling uncertainty and do not cover the redlink gap.
